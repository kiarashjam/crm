...
| 3 | **Connect CRM (optional)** | After sign-in, may be prompted or go to Connection: “Connect your CRM” or “Skip for now.” If they connect, they mark connection as active (account email stored). Then “Continue” → Onboarding. | `/connect` — Connection |
| 4 | **Set up their company (brand)** | Onboarding: enter **Company Name** and **Brand Tone** (Professional / Friendly / Persuasive). This is **their company** for AI copy and branding only (saved in user settings). Save → Dashboard. | `/onboarding` — Onboarding |
| 5 | **Use the app** | User lands on Dashboard and can do all CRM and copy flows below. | `/dashboard` — Dashboard |

**Summary — Beginning:**  
One user = one account. They **sign up** → (optionally) **connect CRM** → **set company name and brand tone** (their company for copy) → **Dashboard**. There is no separate “organization” sign-up; the “company” in onboarding is the name and tone used when generating copy.
...
|------------|--------|-------|
| **Invite users to an organization** | ❌ Not implemented | No “organization” or “workspace”; no invite-by-email. |
| **Add team members** | ❌ Not implemented | No team entity; no “add member” or roles (admin/member). |
| **Share data with other users** | ❌ Not implemented | Each user sees only their own data (backend filters by `UserId`). |
| **Multi-user company account** | ❌ Not implemented | One login = one user; no “company account” with multiple logins. |

**Summary — Inviting/adding people:**  
Today, **only one user per account**. There is **no flow to invite or add new people** to a shared company or team. To support that, the app would need: organizations/workspaces, invite flow, and possibly roles (e.g. admin, member). This is **not in scope** of the current implementation.
...
| **CRM connection** | See connected or not; “Connect” / “Reconnect” → Connection page. | `/settings` → `/connect` |
...
| Flow | What the user does | Route / Page |
|------|--------------------|--------------|
| **Help** | Read how the tool works, how to generate copy, CRM connection, data & privacy, contact support. | `/help` |
