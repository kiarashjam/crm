...
- **Lead:** Entity (name, email, phone, companyId, source, status); CRUD API (`/api/leads` — GET list, GET search, GET by id, POST create, PUT update).
- **Company:** Companies API (`/api/companies` — GET list, GET by id). CompanyRepository + CompanyService + CompaniesController.
...
- **API modules:** leads (getLeads, searchLeads, createLead, updateLead), companies (getCompanies), tasks (getTasks, createTask, updateTask), activities (getActivities, getActivitiesByContact, getActivitiesByDeal, createActivity), reporting (getDashboardStats); deals (createDeal, updateDeal); contacts (phone in map).
- **Pages:** Leads (`/leads` — list, search, create/edit/**delete**, company/source/status); Pipeline (`/pipeline` — Kanban by stage, move stage + isWon, new deal, **delete**); Tasks (`/tasks` — list, All/Pending/Overdue, create/edit with link to lead/deal, complete toggle); Activities (`/activities` — list, log activity, filter by contact/deal for timeline); **Contacts** (`/contacts` — list and search); Companies (`/companies` — list, search, **create, edit**). Dashboard shows getDashboardStats row (active leads, active deals, pipeline value, won/lost).

**Remaining:**

- **Migration:** Run `dotnet ef migrations add SalesCrmCore --project ACI.Infrastructure --startup-project ACI.WebApi` and apply to create Leads, TaskItems, Activities tables and new Deal/Contact columns. See [FLOWS_BACKEND_DATABASE_VERIFICATION.md](FLOWS_BACKEND_DATABASE_VERIFICATION.md) for full flow–backend–database check and exact commands.
- **User roles:** No rep/manager/admin or team access (deferred).
...
| Requirement | Have | Don't Have |
|-------------|------|------------|
| Centralized database of contacts and companies | Contacts: API (list, search), Send to CRM. **Companies API** (GET list, GET by id). **Companies page** (`/companies`) — list and search (read-only). | No company CRUD (create/update) in backend or UI. |
...
**Current:** Contacts and company link in data; Companies API; Companies page (`/companies`) list and search (read-only). Activity entity + API; Activities page (list, log activity, filter by contact/deal for timeline).
...
| Requirement | Have | Don't Have |
|-------------|------|------------|
| Create deals linked to contacts/companies | **Pipeline** new deal UI (name, value, stage, expected close date, **company**); createDeal API; Deal.companyId. | No contact link on deal (contactId not on deal). |
...
| Requirement | Have | Don't Have |
|-------------|------|------------|
| Stage-based pipeline (e.g. Prospect → Qualified → Negotiation → Closed) | Deal has optional `stage` (string); mock uses Qualification, Proposal, Negotiation, Closed Won. | No fixed stage list or workflow; free text only. |
...
**Current system:** Implements core Sales CRM: **Leads** (list/create/edit/**delete**), **Pipeline** (deals by stage, move stage, new deal, **delete**), **Tasks** (list/create/edit, pending/overdue), **Activities** (list, log call/meeting/email/note with optional contact/deal), **Contacts** (list, search at `/contacts`), **Companies** (list, search, **create, edit**), **Reporting** (dashboard stats). Plus AI copy generation, Send to CRM, copy history, auth (including 2FA). Remaining gaps: **User roles** (deferred), last-interaction visibility, contact–deal link, convert lead.
