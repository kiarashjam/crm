...
| **Public pages** | Homepage, Login, Onboarding | Forgot password link; tone examples on Onboarding |
...
| **Where used** | SendToCrm, Leads, Pipeline, Tasks, Activities, Contacts, Companies, Templates, History, Settings. |
...
| C4 | P1 | On full-page loads (Settings initial), wrap spinner in a centered container with short text: "Loading…". |
...
| **Where used** | All app pages except Homepage, Login, Onboarding, Privacy, Terms. |
...
| C9 | P1 | In user dropdown, show current organization and "Switch org" link to `/organizations`. |
...
| Attribute | Detail |
|-----------|--------|
| **Meaning** | Brand (company name, tone); Organization (switcher, invite/join for owner); Security (2FA); Logout; Delete account (two-click confirm). |
| **API** | getUserSettings, saveUserSettings, twoFactorSetup, twoFactorEnable, twoFactorDisable; organization APIs for org switcher and owner invite/join. |
...
| Attribute | Detail |
|-----------|--------|
| **Meaning** | What Cadence does; how to generate copy; your data in Cadence (no external CRM); data & privacy; contact support. |
| **API** | None. |
...
### 3.17 Privacy (`/privacy`) & 3.18 Terms (`/terms`)
...
| P7 | (Removed: Connection page removed) |
...
| P8, P10 | Onboarding: back to Settings; Skip |
...
| C4 | Done — Settings: loading message with label |
| C8 | Done — AppHeader: `aria-current="page"` |
| C9 | Done — AppHeader: current org name + "Switch org" link to `/organizations` in user dropdown |
...
| P11, P12, P15 | Dashboard | Default goal; "Try a template" in empty state |
