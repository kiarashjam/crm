...
| **Organizations** | ✅ | OrganizationsController, InvitesController, JoinRequestsController | OrganizationService, InviteService, JoinRequestService | OrganizationRepository, InviteRepository, JoinRequestRepository | Organization, OrganizationMember, Invite, JoinRequest, OrgSettings | Organizations, OrganizationMembers, Invites, JoinRequests, OrgSettings | ✅ AddOrganizationsAndOrgId |
| **Pipelines** | ✅ | PipelinesController | PipelineService | PipelineRepository | Pipeline | Pipelines | ✅ BlueprintPipelineStagesAndFields |
| **Deal stages** | ✅ | DealStagesController | DealStageService | DealStageRepository | DealStage | DealStages | ✅ BlueprintPipelineStagesAndFields |
| **Lead sources / statuses** | ✅ | LeadSourcesController, LeadStatusesController | LeadSourceService, LeadStatusService | (repos) | LeadSource, LeadStatus | LeadSources, LeadStatuses | ✅ BlueprintPipelineStagesAndFields |
| **Leads convert** | ✅ | LeadsController (POST …/convert) | LeadService.ConvertAsync | — | — | — | N/A |

**Note:** Later migrations (e.g. SalesCrmCore, AddOrganizationsAndOrgId, RemoveCrmConnection, BlueprintPipelineStagesAndFields) add Leads, TaskItems, Activities, OrganizationId on data entities, Pipelines, DealStages, LeadSources, LeadStatuses, Company (Domain, Industry, Size), Contact/Company delete, Deal (PipelineId, DealStageId, Currency, AssigneeId, ContactId), Lead (LeadSourceId, LeadStatusId, IsConverted, ConvertedAtUtc), and remove CrmConnections. Ensure all migrations are applied (`dotnet ef database update`).
...
### 5.5 Copy history (CopyHistoryController)
...
### 8.1 New user: sign up → organizations → onboarding → dashboard

1. **Register** → POST /api/auth/register → AuthService → UserRepository (insert User, UserSettings) → JWT + user.
2. **Organizations** → GET /api/organizations, POST /api/organizations (create) or accept invite → open org.
3. **Onboarding** → PUT /api/settings { companyName, brandTone } → SettingsService → UserSettings update.
4. **Dashboard** → GET /api/reporting/dashboard, GET /api/copyhistory/stats, GET /api/copyhistory, GET /api/templates. (Connection API removed.) Reporting fails if Leads table missing.
...
- **Application services:** Registered in Program.cs (Auth, Contact, Deal, Lead, Company, Task, Activity, Template, CopyHistory, Settings, Organization, Invite, JoinRequest, CopyGenerator, SendToCrm). (Connection removed.)
...
*Verified against: backend Domain, Application, Infrastructure, WebApi (entities, DTOs, interfaces, services, repositories, configurations, migrations, Program.cs, DependencyInjection); frontend api/* and page usage. Build status: 0 errors, 0 warnings. Tests: 169 unit tests passing across 10 services. Validation: DataAnnotations on all DTOs + ValidationHelper. Last updated: February 2026.*
