...
This report checks that the Cadence frontend has everything needed from a **CRM sales** perspective: contacts, deals, copy types, send-to-CRM, history, and sales workflows (Cadence-only; no external CRM connection). The app is **standalone**: it can run with no external service (demo mode or full stack locally—see [RUN_FROM_SCRATCH.md](../../RUN_FROM_SCRATCH.md)).
...
### Sending copy (Cadence-only)

| Item | Status | Where |
|------|--------|--------|
| **No external CRM** | Yes | Cadence is the only CRM; no Connection page. Data stays in Cadence. |
...
| Item | Status | Where |
|------|--------|--------|
| **Dashboard** | Yes | Main hub: stats, copy types, goal, context, length, generate → Generated Copy |
| **Templates** | Yes | Templates page → “Use template” pre-fills Dashboard |
| **Settings** | Yes | Brand (company name, tone), Organization (switcher, invite/join for owner), Security (2FA enable/disable), Logout, Delete account |
| **Help** | Yes | How it works, generate steps, your data in Cadence, data & privacy, support |
...
### 4. Organization in nav

- **Current:** AppHeader shows current org name and “Switch org” link to /organizations. Settings has Organization section (switcher, invite/join for owner). No Connection page — Cadence is the only CRM.
- **Recommendation:** None; org flow implemented.
...
- **Send to Contact/Deal** (Cadence-only) with history and recent activity.
- **Brand/company settings** used in the Intelligent Sales Writer.
