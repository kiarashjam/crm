...
| **Intelligent Sales Writer** | Template-based, in-process. No external LLM or API. |
| **External CRM** | Not required. No third-party CRM needed to use the app. |
...
| **Data isolation** | Backend filters CRM data by `UserId` and, when `X-Organization-Id` header is set, by `OrganizationId`. Organizations, memberships, invites, join requests exist in the backend; frontend has org flow (create/join/open org, org switcher in Settings and AppHeader, owner invite/join-request UI). |
...
| Aspect | Where it's documented | Summary |
|--------|----------------------|---------|
| **All routes and pages** | [src/app/reports/FRONTEND_PAGES_REPORT.md](./src/app/reports/FRONTEND_PAGES_REPORT.md) | 18 routes: `/` Homepage, `/login`, `/organizations`, `/onboarding`, `/dashboard`, `/generated`, `/send`, `/leads`, `/deals` (Pipeline), `/tasks`, `/activities`, `/contacts`, `/companies`, `/templates`, `/history`, `/settings`, `/help`, `/privacy`, `/terms`. (No `/connect`.) |
...
| **API overview** | backend/README.md, [FLOWS_BACKEND_DATABASE_VERIFICATION.md](./src/app/reports/FLOWS_BACKEND_DATABASE_VERIFICATION.md) | Auth, Settings, Templates, Copy (generate/send), Copy history, Contacts, Companies, Deals, Leads, Tasks, Activities, Reporting (dashboard stats). (Connection/CrmConnection removed.) |
| **Database schema and migrations** | FLOWS_BACKEND_DATABASE_VERIFICATION.md | Tables: Users, UserSettings, **Organizations, OrganizationMembers, Invites, JoinRequests, OrgSettings**, Companies, Contacts, Deals, Leads, TaskItems, Activities, Templates, CopyHistoryItems. CrmConnections table removed (migration RemoveCrmConnection). Data entities have nullable OrganizationId; backfill on create-org. |
...
| Aspect | Where it's documented | Summary |
|--------|----------------------|---------|
| **New user journey** | [src/app/reports/USER_FLOWS_REPORT.md](./src/app/reports/USER_FLOWS_REPORT.md) | Homepage → Login (or demo) → Organizations (create/join/open org) → Onboarding (company name, brand tone) → Dashboard. |
| **"Their company"** | Same report | User's company = name + brand tone (settings). Companies in CRM = accounts they track (customer companies). Target vision (report 01): organizations; backend has org APIs and org-scoped data; frontend org flow implemented (create/join/open org, switcher, owner invite/join-request UI). |
| **All CRM flows** | Same report | Copy (generate, templates, adjust, send to CRM, history); Leads (list, search, add, edit, delete); Pipeline/Deals (Kanban, move stage, new deal, delete); Tasks (list, filter, add, edit, complete); Activities (list, log, filter by contact/deal); Companies (list, add, edit); Contacts (list, search); Dashboard stats; Settings (brand, org switcher, 2FA, logout, delete); Help, Privacy, Terms. |

---

## 9. Intelligent Sales Writer and send-to-CRM
...
|--------|------------------|
| Repo root | [README.md](./README.md), [RUN_FROM_SCRATCH.md](./RUN_FROM_SCRATCH.md), [PROJECT_ASPECTS.md](./PROJECT_ASPECTS.md) (this file) |
...
| docs/ | [docs/README.md](./docs/README.md) — consolidated documentation |
...
## 14. Recent Improvements (February 2026)

### Backend Quality Enhancements ✅
The backend has been significantly improved to reach **92% production readiness**:

| Category | Score | Status |
|----------|-------|--------|
| Architecture | 90% | ✅ Excellent |
| Error Handling | 95% | ✅ Result pattern in all major services |
| Logging | 95% | ✅ Serilog with context enrichment |
| API Documentation | 100% | ✅ All 26 controllers documented |
| Global Exception Handler | - | ✅ ProblemDetails format |
| Testing | 169 unit tests | ✅ Complete |

**Key Files:** `ACI.Application/Common/Result.cs`, `DomainErrors.cs`, `ACI.WebApi/Middleware/GlobalExceptionHandler.cs`

### Frontend Quality Enhancements ✅
- TypeScript errors fixed (100+ resolved)
- TanStack Query + Vitest + MSW testing infrastructure
- Code splitting with lazy loading
- ESLint + Prettier configured
- Large file refactoring: Dashboard.tsx (41% reduction ✅), Homepage.tsx (23% reduction ✅)

**See full details:** `src/app/reports/BACKEND_CODE_QUALITY_AND_STANDARDS_REPORT.md` and `FRONTEND_QUALITY_IMPROVEMENT_REPORT.md`
